...
By default, when you join to a channel you won't listen any event except of `ChannelChatMessage`. To listen to the other events you will need to specify them when joining to a channel.
...
Here is an example of how get that channel profile and subscribe manage its events:
...
```js ins={9}
const { ChatBot } = require('@twitchapi/chatbot')
...
import { createEvent } from "@twitchapi/chatbot";
...
const { createEvent } = require('@twitchapi/chatbot');
