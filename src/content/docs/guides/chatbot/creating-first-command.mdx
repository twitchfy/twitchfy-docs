...
For creating a command you will have to simply create a class and export it. The class has to extend a pre-defined class called [Command](/api/chatbot/classes/command). 
...
import { Command } from '@twitchapi/chatbot';
...
const { Command } = require('@twitchapi/chatbot'); //using cjs
...
```ts showLineNumbers copy title="commands/hello.ts"
import { Command, SetCommand } from '@twitchapi/chatbot';
...
```ts showLineNumbers copy title="commands/hello.js"
const { Command } = require('@twitchapi/chatbot');
...
Once you have set the name of the command it's time to run the command logic. When an user runs the command the package will try to execute the `run` method from the command. 
...
```ts showLineNumbers copy title="commands/hello.ts"
import { Command, SetCommand, type CommandContext } from '@twitchapi/chatbot';
...
import { ChatBot } from '@twitchapi/chatbot'
