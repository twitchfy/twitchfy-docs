...
First of all we have to install `@twitchfy/chatbot` package with our custom Node Package Manager.
...
declare module '@twitchfy/chatbot' {
  interface Options {
    connection: EventSubConnection.Webhook // or WebSocket if you are using websocket
  }
}
```
:::
...
If you have an storage set into your chatbot's eventsub you won't need to join everytime you start your chatbot to the same channels.
