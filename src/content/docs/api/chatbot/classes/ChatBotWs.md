...
### new ChatBotWs(chatbot, nick, auth)

```ts
new ChatBotWs(
   chatbot: ChatBot, 
   nick: string, 
   auth: UserTokenAdapter<boolean>): ChatBotWs
```
...
[twitchapi/packages/chatbot/src/websocket/ChatBotWs.ts:53](https://github.com/pablornc/twitchapi//blob/b274026/packages/chatbot/src/websocket/ChatBotWs.ts#L53)
...
| `connection` | `public` | `connection` | **Description**<br />The WebSocket connection to the Twitch IRC Server. | - |
| `nick` | `public` | `string` | **Description**<br />The nick of the ChatBot used to login with the Twitch IRC Server | - |
...
connect(
   ...
   extraRequestOptions?: RequestOptions): void
```
...
| Parameter | Type | Description |
| :------ | :------ | :------ |
| `requestUrl` | `string` \| `Url` | should be a standard websocket url |
| `requestedProtocols`? | `string` \| `string`[] | list of subprotocols supported by the client.<br />    The remote server will select the best subprotocol that it supports and send that back when establishing the connection. |
| `origin`? | `string` | Used in user-agent scenarios to identify the page containing<br />    any scripting content that caused the connection to be requested. |
| `headers`? | `OutgoingHttpHeaders` | additional arbitrary HTTP request headers to send along with the request.<br />    This may be used to pass things like access tokens, etc. so that the server can verify authentication/authorization<br />    before deciding to accept and open the full WebSocket connection. |
| `extraRequestOptions`? | `RequestOptions` | additional configuration options to be passed to `http.request` or `https.request`.<br />    This can be used to pass a custom `agent` to enable `client` usage from behind an HTTP or HTTPS proxy server<br />    using [https://github.com/koichik/node-tunnel\|koichik/node-tunnel](https://github.com/koichik/node-tunnel\|koichik/node-tunnel) or similar. |
...
twitchapi/node\_modules/@types/websocket/index.d.ts:630
...
set by `emitter.setMaxListeners(n)` or defaults to [defaultMaxListeners](ChatBotWs.md).
...
Returns the ping of the websocket connnection's ping in ms.

#### Source

[twitchapi/packages/chatbot/src/websocket/ChatBotWs.ts:90](https://github.com/pablornc/twitchapi//blob/b274026/packages/chatbot/src/websocket/ChatBotWs.ts#L90)
...
Returns the current instance of the WebSocket Client used to connect to Twitch IRC Server.

#### Source

[twitchapi/packages/chatbot/src/websocket/ChatBotWs.ts:67](https://github.com/pablornc/twitchapi//blob/b274026/packages/chatbot/src/websocket/ChatBotWs.ts#L67)
...
| Parameter | Type | Description |
| :------ | :------ | :------ |
| `message` | `string` | Returns the message sent to the Twitch IRC Server. |
...
[twitchapi/packages/chatbot/src/websocket/ChatBotWs.ts:81](https://github.com/pablornc/twitchapi//blob/b274026/packages/chatbot/src/websocket/ChatBotWs.ts#L81)
